...
class TestAuth(AppTestCase):
  ...
  def can_update_password(self):
    resp = self._update_password(MOCK_USER_NO_PERM['username'], MOCK_USER_NO_PERM['registration_number'], MOCK_LOGIN_NO_PERM['password'])
    self.assertEqual(resp.status_code, 200)

    resp = self._update_password(MOCK_USER['username'], MOCK_USER['registration_number'], MOCK_LOGIN['password'])
    self.assertEqual(resp.status_code, 200)

  def _update_password(self, username, registration_number, password):
    resp = self.client.post(API_ROUTE, data={'username': username, 'password': str(registration_number) })
    header = resp.get_json()

    resp = self.client.post(API_ROUTE + '/password', headers=header, data={'password': password})
    return resp

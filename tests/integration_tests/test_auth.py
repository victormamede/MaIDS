...
class TestAuth(AppTestCase):
    def setUp(self):
        ...
        self.client.post(
            "/api/user", data=MOCK_USER_NO_PERM, headers=self.master_header
        )
    ...
    def cannot_auth_with_wrong_credentials(self):
        ...
        WRONG_CREDENTIALS = {"username": "wrong_username", "password": "wrong_password"}
    ...
    def can_update_password(self):
        resp = self._update_password(
            MOCK_USER_NO_PERM["username"],
    ...
    def _update_password(self, username, registration_number, password):
        resp = self.client.post(
            API_ROUTE, data={"username": username, "password": str(registration_number)}
        )
        header = resp.get_json()

        resp = self.client.post(
            API_ROUTE + "/password", headers=header, data={"password": password}
        )
        return resp

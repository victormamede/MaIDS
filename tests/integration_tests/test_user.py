...
class TestUser(AppTestCase):
    ...
    def gets_all_users(self, client):
        ...
        self.assertIn(MOCK_USER['username'],
                      [user['username'] for user in resp.get_json()])
    ...
    def changes_user(self, client):
        ...
        resp = client.put(API_ROUTE + '/' + str(self.mock_user_id),
                          data=to_change)
        json = resp.get_json()
    ...
    def filters_user_list(self, client):
        ...
        resp = client.get(API_ROUTE,
                          query_string={'username': MOCK_USER_2['username']})

...
class PermissionMocker():
    ...
    def __enter__(self):
        self.perm_user = MockUserClient([self.role], self.parent,
                                        PERMISSION_MOCK_USER).__enter__()
        self.no_perm_user = MockUserClient(
            [], self.parent, NO_PERMISSION_MOCK_USER).__enter__()
